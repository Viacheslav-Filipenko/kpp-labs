...
class ApiService {
	...
	async getWeaterByCityName(city, country) {
		...
		const response = await fetch(
			`https://api.openweathermap.org/data/2.5/weather?q=${q}&appid=${this.apiKey}`
		);
...
class MapService {
	constructor(map, config) {
		this.map = new google.maps.Map(map, config);
	}
...
const main = async () => {
	...
	const mapService = new MapService(document.getElementById('map'), mapConfig);
